...
void foo() {
  ...
  DLIO_PROFILER_C_FUNCTION_UPDATE_INT("key", 0);
  DLIO_PROFILER_C_FUNCTION_UPDATE_STR("key", "0");
  sleep(1);
  DLIO_PROFILER_C_REGION_START(CUSTOM);
  DLIO_PROFILER_C_REGION_UPDATE_INT(CUSTOM, "key", 0);
  DLIO_PROFILER_C_REGION_UPDATE_STR(CUSTOM, "key", "0");
  sleep(1);
