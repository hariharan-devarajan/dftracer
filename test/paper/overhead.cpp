...
int main(int argc, char* argv[]) {
  ...
  if (argc < 4) {
    DLIO_PROFILER_LOGERROR("usage: overhead FILENAME <NUM OPERATIONS> <TRANSFER SIZE>", "");
    exit(1);
  }
  ...
  if (my_rank == 0) {
    DLIO_PROFILER_LOGINFO("Running with transfer size %lu  and num ops %lu", transfer_size, num_operations);
  }
  long double file_size = num_operations * transfer_size;
  int ts = ceil(file_size  / comm_size);
  if (my_rank == 0) {
    DLIO_PROFILER_LOGINFO("Writing %d per rank for data generation", ts);
  }
  sprintf(filename_primary, "%s/file_0-%d.bat", argv[2], comm_size);
  {
    MPI_File fh_orig;
    int status_orig = MPI_File_open(MPI_COMM_WORLD, filename_primary,
    ...
    if (written_bytes != ts) {
      DLIO_PROFILER_LOGERROR("Write was unsuccessful written %d of %d bytes", written_bytes, ts);
    }
    assert(written_bytes == ts);
    status_orig = MPI_File_close(&fh_orig);
  }
  if (my_rank != 0) {
    std::string cmd = "cp " + std::string(filename_primary) + " " + std::string(filename);
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  if (my_rank == 0) {
    DLIO_PROFILER_LOGINFO("Created dataset for test", "");
  }
  Timer operation_timer;
  MPI_Barrier(MPI_COMM_WORLD);
  if (my_rank == 0) {
    DLIO_PROFILER_LOGINFO("Starting read", "");
  }
  ...
  for (uint64_t i = 0; i < num_operations; ++i) {
    ...
    operation_timer.pauseTime();
    assert(transfer_size == read_bytes);
    MPI_Barrier(MPI_COMM_WORLD);
    if (i % 10000 == 0 && my_rank == 0) {
      DLIO_PROFILER_LOGINFO("Completed %d loops", i);
    }
  }
  free(buf);
  operation_timer.resumeTime();
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  if (my_rank == 0) {
    DLIO_PROFILER_LOGINFO("Finishing read", "");
  }
  ...
  if (my_rank == 0) {
    DLIO_PROFILER_LOGPRINT("scale,ops,ts,time", "")
    DLIO_PROFILER_LOGPRINT("%d,%lu,%lu,%f", comm_size, num_operations, transfer_size, total_time)
  }
