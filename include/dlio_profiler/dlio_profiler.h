...
#define DLIO_PROFILER_CPP_FINI() finalize()
#define DLIO_PROFILER_CPP_FUNCTION() \
  DLIOProfiler profiler_dlio_fn =    \
      DLIOProfiler((char *)__FUNCTION__, CPP_LOG_CATEGORY);
...
#define DLIO_PROFILER_CPP_REGION_END(name) delete profiler_##name
...
#define DLIO_PROFILER_C_INIT_NO_BIND(log_file, data_dirs, process_id) \
  initialize_no_bind(log_file, data_dirs, process_id);
#define DLIO_PROFILER_C_FINI() finalize()

#define DLIO_PROFILER_C_FUNCTION_START() \
  struct DLIOProfilerData *data_fn =     \
      initialize_region(__func__, C_LOG_CATEGORY);

#define DLIO_PROFILER_C_FUNCTION_END() finalize_region(data_fn);

#define DLIO_PROFILER_C_REGION_START(name) \
  struct DLIOProfilerData *data_##name =   \
      initialize_region(#name, C_LOG_CATEGORY);

#define DLIO_PROFILER_C_REGION_END(name) finalize_region(data_##name);
