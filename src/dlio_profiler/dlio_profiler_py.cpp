...
TimeResolution get_time() {
  ...
  if (dlio_profiler_inst != nullptr)
    return dlio_profiler_inst->get_time();
  else
    DLIO_PROFILER_LOGDEBUG("py.get_time dlio_profiler not initialized", "");
  return 0;
}
...
void log_event(std::string name, std::string cat, TimeResolution start_time,
  ...
  auto dlio_profiler_inst =
      dlio_profiler::Singleton<dlio_profiler::DLIOProfilerCore>::get_instance(
          ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_PY_APP);
  if (dlio_profiler_inst != nullptr)
    dlio_profiler_inst->log(name.c_str(), cat.c_str(), start_time, duration,
                            &args);
  else
    DLIO_PROFILER_LOGDEBUG("py.log_event dlio_profiler not initialized", "");
}

void enter_event() {
  DLIO_PROFILER_LOGDEBUG("py.enter_event", "");
  auto dlio_profiler_inst =
      dlio_profiler::Singleton<dlio_profiler::DLIOProfilerCore>::get_instance(
          ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_PY_APP);
  dlio_profiler_inst->enter_event();
}

void exit_event() {
  DLIO_PROFILER_LOGDEBUG("py.exit_event", "");
  auto dlio_profiler_inst =
      dlio_profiler::Singleton<dlio_profiler::DLIOProfilerCore>::get_instance(
          ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_PY_APP);
  dlio_profiler_inst->exit_event();
}
...
PYBIND11_MODULE(dlio_profiler_py, m) {
  ...
  m.def("enter_event", &dlio_profiler::enter_event, "mark enter event");
  m.def("exit_event", &dlio_profiler::exit_event, "mark exit event");
