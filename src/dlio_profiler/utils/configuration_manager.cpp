...
dlio_profiler::ConfigurationManager::ConfigurationManager()
  ...
  if (env_conf != nullptr) {
    ...
    if (config[DLP_YAML_PROFILER]) {
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.logger_level %d",
                             this->logger_level);
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.init_type %d",
                             this->init_type);
      if (config[DLP_YAML_PROFILER][DLP_YAML_PROFILER_LOG_FILE]) {
        this->log_file = config[DLP_YAML_PROFILER][DLP_YAML_PROFILER_LOG_FILE]
                             .as<std::string>();
      }
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.log_file %s",
                             this->log_file.c_str());
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.data_dirs_str %s",
                             this->data_dirs.c_str());
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.trace_all_files %d",
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.compression %d",
                             this->compression);
    }
    if (config[DLP_YAML_GOTCHA]) {
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.gotcha_priority %d",
                             this->gotcha_priority);
    }
    if (config[DLP_YAML_FEATURES]) {
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.metadata %d",
                             this->metadata);
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.core_affinity %d",
                             this->core_affinity);
      if (config[DLP_YAML_FEATURES][DLP_YAML_FEATURES_IO] &&
          config[DLP_YAML_FEATURES][DLP_YAML_FEATURES_IO_ENABLE]) {
        this->io = config[DLP_YAML_FEATURES][DLP_YAML_FEATURES_IO]
                         [DLP_YAML_FEATURES_IO_ENABLE]
                             .as<bool>();
        ...
        if (this->io) {
          ...
          DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.posix %d",
                                 this->posix);
          if (config[DLP_YAML_FEATURES][DLP_YAML_FEATURES_IO]
          ...
          DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.stdio %d",
                                 this->stdio);
        }
      }
    ...
    if (config[DLP_YAML_INTERNAL]) {
      ...
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.bind_signals %d",
                             this->bind_signals);
      if (config[DLP_YAML_INTERNAL][DLP_YAML_INTERNAL_THROW_ERROR]) {
        this->throw_error =
            config[DLP_YAML_INTERNAL][DLP_YAML_INTERNAL_THROW_ERROR].as<bool>();
      }
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.throw_error %d",
                             this->throw_error);
      if (config[DLP_YAML_INTERNAL][DLP_YAML_INTERNAL_WRITE_BUFFER_SIZE]) {
        this->write_buffer_size =
            config[DLP_YAML_INTERNAL][DLP_YAML_INTERNAL_WRITE_BUFFER_SIZE]
                .as<size_t>();
      }
      DLIO_PROFILER_LOGDEBUG("YAML ConfigurationManager.write_buffer_size %d",
                             this->write_buffer_size);
    }
  }
  ...
  if (this->enable) {
    ...
    DLIO_PROFILER_LOGDEBUG("ENV ConfigurationManager.gotcha_priority %d",
                           this->gotcha_priority);
    const char *env_log_file = getenv(DLIO_PROFILER_LOG_FILE);
    if (env_log_file != nullptr) {
      this->log_file = env_log_file;
    }
    DLIO_PROFILER_LOGDEBUG("ENV ConfigurationManager.log_file %s",
                           this->log_file.c_str());
    ...
    DLIO_PROFILER_LOGDEBUG("ENV ConfigurationManager.data_dirs %s",
                           this->data_dirs.c_str());
    DLIO_PROFILER_LOGDEBUG("ENV ConfigurationManager.trace_all_files %d",
                           this->trace_all_files);
    ...
    DLIO_PROFILER_LOGDEBUG("ENV ConfigurationManager.throw_error %d",
                           this->throw_error);
    ...
    DLIO_PROFILER_LOGDEBUG("ENV ConfigurationManager.compression %d",
                           this->compression);
    ...
    DLIO_PROFILER_LOGDEBUG("ENV ConfigurationManager.write_buffer_size %d",
                           this->write_buffer_size);
  }
