...
void initialize(const char * log_file, const char * data_dirs, int* process_id) {
  DLIO_PROFILER_MAIN_SINGLETON_INIT(ProfilerStage::PROFILER_INIT, ProfileType::PROFILER_C_APP, log_file, data_dirs, process_id);
}
...
void log_event(const char * name, const char * cat, TimeResolution start_time, TimeResolution duration) {
  ...
  DLIO_PROFILER_MAIN_SINGLETON(ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_C_APP)->log(name, cat, start_time, duration, args);
}
