...
DLIOProfiler::DLIOProfiler(ConstEventType _name, ConstEventType _cat)
    : initialized(true), name(_name), cat(_cat), metadata(nullptr) {
  DLIO_PROFILER_LOGDEBUG("DLIOProfiler::DLIOProfiler event %s cat %s ", _name,
                         _cat);
  auto dlio_profiler_core = DLIO_PROFILER_MAIN_SINGLETON(
      ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_CPP_APP);
  if (dlio_profiler_core != nullptr) {
    if (dlio_profiler_core->include_metadata)
...
void DLIOProfiler::update(const char *key, int value) {
  DLIO_PROFILER_LOGDEBUG(
      "DLIOProfiler::update event %s cat %s  key %s value %d ", name, cat, key,
      value);
  auto dlio_profiler_core = DLIO_PROFILER_MAIN_SINGLETON(
      ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_CPP_APP);
  if (dlio_profiler_core != nullptr && dlio_profiler_core->is_active() &&
      dlio_profiler_core->include_metadata) {
    metadata->insert_or_assign(key, value);
  }
}
void DLIOProfiler::update(const char *key, const char *value) {
  DLIO_PROFILER_LOGDEBUG(
      "DLIOProfiler::update event %s cat %s  key %s value %s ", name, cat, key,
      value);
  auto dlio_profiler_core = DLIO_PROFILER_MAIN_SINGLETON(
      ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_CPP_APP);
  if (dlio_profiler_core != nullptr && dlio_profiler_core->is_active() &&
...
DLIOProfiler::~DLIOProfiler() {
  DLIO_PROFILER_LOGDEBUG("DLIOProfiler::~DLIOProfiler event %s cat %s", name,
                         cat);
...
void initialize_main(const char *log_file, const char *data_dirs,
  ...
  DLIO_PROFILER_MAIN_SINGLETON_INIT(ProfilerStage::PROFILER_INIT,
                                    ProfileType::PROFILER_C_APP, log_file,
...
void initialize_no_bind(const char *log_file, const char *data_dirs,
  ...
  DLIO_PROFILER_MAIN_SINGLETON_INIT(ProfilerStage::PROFILER_OTHER,
                                    ProfileType::PROFILER_C_APP, log_file,
                                    data_dirs, process_id);
}

struct DLIOProfilerData *initialize_region(ConstEventType name,
                                           ConstEventType cat) {
  DLIO_PROFILER_LOGDEBUG("dlio_profiler.initialize_region event %s cat %s",
                         name, cat);
...
void update_metadata_int(struct DLIOProfilerData *data, const char *key,
                         int value) {
...
void update_metadata_string(struct DLIOProfilerData *data, const char *key,
                            const char *value) {
...
TimeResolution get_time() {
  ...
  auto dlio_profiler = DLIO_PROFILER_MAIN_SINGLETON(
      ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_C_APP);
  if (dlio_profiler != nullptr)
    return dlio_profiler->get_time();
  else
    DLIO_PROFILER_LOGERROR(
        "dlio_profiler.cpp.get_time dlio_profiler not initialized", "");
  return 0;
}
...
void log_event(ConstEventType name, ConstEventType cat,
  ...
  auto dlio_profiler = DLIO_PROFILER_MAIN_SINGLETON(
      ProfilerStage::PROFILER_OTHER, ProfileType::PROFILER_C_APP);
  if (dlio_profiler != nullptr)
    dlio_profiler->log(name, cat, start_time, duration, nullptr);
  else
    DLIO_PROFILER_LOGERROR(
        "dlio_profiler.cpp.log_event dlio_profiler not initialized", "");
}

void finalize() {
  DLIO_PROFILER_LOGDEBUG("dlio_profiler.cpp.finalize", "");
  auto dlio_profiler = DLIO_PROFILER_MAIN_SINGLETON(
      ProfilerStage::PROFILER_FINI, ProfileType::PROFILER_C_APP);
  if (dlio_profiler != nullptr) {
